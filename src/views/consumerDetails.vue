    ...
    <div class="card-body d-flex align-items-center py-8">
      ...
      <div class="ms-12 col-lg-12">
        <div class="list-unstyled text-gray-600 fw-bold fs-6 p-0 m-0 row">
          <div class="col-lg-2 fw-bold text-muted">Concurrency :</div>
          <div class="col-lg-10 fw-bolder fs-6 text-gray-800">
            <span class="bullet bullet-dot bg-primary me-5"></span>
            {{ consumer.concurrency }}
          </div>
          <div class="col-lg-2 fw-bold text-muted">Last Active at :</div>
          <div class="col-lg-10 fw-bolder fs-6 text-gray-800">
            <span class="bullet bullet-dot bg-primary me-5"></span>
        ...
        <div class="list-unstyled text-gray-600 fw-bold fs-6 p-0 m-0 row">
          ...
          <div class="col-lg-8">
            <div
              ...
              v-for="topic in consumer.topics"
              :key="topic"
            >
              <span class="bullet bullet-dot bg-primary me-5"></span>
              {{ topic }}
      ...
      >
        <li class="nav-item">
          <a
            ...
            role="tab"
          >
            <span style="margin-right: 5px"> Pending </span
            ><span class="badge badge-light-info" v-if="counts.pending">
              {{ counts.pending }}
            </span>
          </a>
        </li>
        <li class="nav-item">
          <a
            ...
            @click="
              setActiveTab($event);
              getStatusData('active');
            "
            data-tab-index="1"
            href="#kt_builder_header"
            role="tab"
          >
            <span style="margin-right: 5px"> Active </span
            ><span class="badge badge-light-success" v-if="counts.active">
              {{ counts.active }}</span
            >
          </a>
        </li>
        <li class="nav-item">
          ...
          >
            ...
            ><span class="badge badge-light-success" v-if="counts.completed">
        ...
        <li class="nav-item">
          <a
            ...
            @click="
              setActiveTab($event);
              getStatusData('delayed');
            "
            data-tab-index="3"
            href="#kt_builder_aside"
            role="tab"
          >
            <span style="margin-right: 5px"> Delayed </span
            ><span class="badge badge-light-warning" v-if="counts.delayed">
              {{ counts.delayed }}</span
            >
          </a>
        </li>
        <li class="nav-item">
          <a
            ...
            role="tab"
          >
            <span style="margin-right: 5px"> Failed </span
            ><span class="badge badge-light-danger" v-if="counts.failed">
              {{ counts.failed }}</span
            >
          </a>
        </li>
        ...
        <div class="tab-pane active" id="kt_builder_main">
          <div class="pull-right">
            <button
              ...
              v-if="
                ...
                statusData.length != 0
              "
              @click="deleteStatuData(status)"
            >
              <em class="bi bi-trash"></em> Delete All
            </button>
            <button
              class="btn btn-sm btn-primary"
              v-if="status === 'failed' && statusData.length != 0"
              @click="retryAll()"
            >
              <em class="bi bi-arrow-repeat" style="font-size: 15px"></em> Retry
              All
            </button>
          </div>
          <div
            v-if="!statusData || statusData.length == 0"
            class="text-muted text-center h3"
          >
            No data available
          </div>
          <div
            v-for="record in statusData"
            :key="record.key"
            class="card bgi-no-repeat h-xl-100"
            style="
          ...
          >
            ...
            <div class="card-body" style="padding: 1rem 1rem">
              <div
                class="card-header"
                style="border-bottom: 0px; min-height: 0px; padding: 0px"
              >
                <a
                ...
                <div class="card-toolbar">
                  <a
                    ...
                    @click="deleteMessage(record.key)"
                  >
              ...
              <div class="history-box">
                <div
                  ...
                  v-for="(item, key) in record.statusHistory"
                  :key="item"
                >
                  {{ key }} : {{ formatDate(item) }}
                </div>
              </div>
              <p class="text-dark-75 fw-bold fs-5 m-0 my-4">
                Message : {{ record.value }}
              </p>
              <p
                class="text-dark-75 fw-bold fs-5 m-0 text-danger my-4"
                v-if="record.error"
              >
                <span v-if="status != 'failed'">Previous </span>Error :
                {{ record.error }}
              </p>
            </div>
          </div>
...
export default defineComponent({
  ...
  mounted() {
    setCurrentPageTitle(
      "Consumer " + this.$route.params.consumerId + " details"
    );
    this.getTopics();
    this.getcounts();
  ...
  methods: {
    ...
    getStatusData(status) {
      ...
      this.status = status;
      ApiService.get("msg/list/json/" + this.consumerId + "/" + status).then(
        (d) => {
